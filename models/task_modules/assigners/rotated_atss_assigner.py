...
class RotatedATSSAssigner(BaseAssigner):
    ...
    def assign(
        ...
        if self.alpha is None:
            ...
            if overlaps.size() == 1:
                overlaps = self.iou_calculator(bbox_preds, gt_bboxes).squeeze()
        ...
        gt_cx, gt_cy = RotatedBoxes(gt_bboxes.tensor).centers.unbind(dim=-1)
        # a = gt_bboxes.tensor[:, 0]
        # np.cos(gt_bboxes.tensor[:, 4])
        # gt_bboxes.tensor[:, 2]
        # # compute center distance between all bbox and gt
        # gt_cx = (gt_bboxes.tensor[:, 0]) + (np.cos(gt_bboxes.tensor[:, 4])*gt_bboxes.tensor[:, 2]) / 2.0
        # gt_cy = gt_bboxes.tensor[:, 1] + (np.cos(gt_bboxes.tensor[:, 4])*gt_bboxes.tensor[:, 3]) / 2.0
        gt_points = torch.stack((gt_cx, gt_cy), dim=1)

        bboxes_cx, bboxes_cy = RotatedBoxes(priors).centers.unbind(dim=-1)
        # bboxes_cy = (priors[:, 1] + priors[:, 3]) / 2.0
        ...
        # mean and std, set mean + std as the iou threshold
        # h=torch.arange(num_gt).unsqueeze(-1)
        ...
        priors_cx, priors_cy = RotatedBoxes(priors).centers.unbind(dim=-1)
