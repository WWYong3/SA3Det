...
from mmdet.models import build_assigner, CrossEntropyLoss
...
from mmdet.structures.bbox import get_box_tensor, distance2bbox
# from ..utils.bbox_utils import filter_scores_and_topk
from mmrotate.structures.bbox import RotatedBoxes
from mmrotate.registry import MODELS

@MODELS.register_module()
# class MyFAMHead(ATSSHead):
class MyFAMHead(RetinaHead):
    ...
    def _init_layers(self):
        ...
        self.reg_offset_module = nn.Conv2d(
            self.feat_channels // self.offset_channel_shrink * 3, self.num_base_priors*reg_dim * 2, 3, padding=1)
        self.pyramid_offset = nn.Conv2d(
            self.feat_channels // self.offset_channel_shrink * 3, self.num_base_priors*reg_dim, 3, padding=1)
    ...
    def forward(self, feats):
        ...
        for idx, (x, scale, stride) in enumerate(
            ...
            anchor = self.prior_generator.single_level_grid_priors(
                (h, w), idx, device=x.device)           #147456,5
            if b!=1:
            ...
            if self.anchor_type == 'anchor_free':
                ...
                reg_dist = reg_dist.permute(0, 2, 3, 1).reshape(-1, 5)
                reg_bbox = distance2bbox(
                    self.anchor_center(anchor) / stride[0],
                    reg_dist).reshape(b, h, w, 5).permute(0, 3, 1,
                                                          2)  # (b, c, h, w)
                ...
                reg_bbox = self.bbox_coder.decode(anchor, reg_dist).reshape(
                    b, h, w, 5*self.num_base_priors).permute(0, 3, 1, 2) / stride[0]
            else:
